...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function performLogin(email, password, rememberMe) {
        ...
        try {
            ...
            // Admin ise session token'ı kaydet
            if (result.sessionToken && window.SessionAuth) {
                window.SessionAuth.setSessionToken(result.sessionToken);
            } else if (window.SessionAuth) {
                window.SessionAuth.setSessionToken('');
            }
    ...
    function continueLoginAfterPasswordChange(userData, rememberMe) {
        ...
        // Admin ise GAS'tan token al
        if (userData.role === 'admin' && window.SessionAuth) {
            fetch(USER_URL + '?action=createToken&email=' + encodeURIComponent(userData.email))
                .then(r => r.json())
                .then(result => {
                ...
                .catch(() => { /* token alınamazsa sessizce devam */ });
        } else if (window.SessionAuth) {
            window.SessionAuth.setSessionToken('');
        }
