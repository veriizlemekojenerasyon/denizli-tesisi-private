...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.SessionAuth) {
        window.SessionAuth.validateAdminSession().then(function(valid) {
            if (!valid) return;
            window.SessionAuth.startTokenWatcher();
            setDefaultDate();
            setupEvents();
            loadAnnouncements();
        });
...
async function saveAnnouncement(event) {
    ...
    if (window.saveAnnouncementToSheets && window.isBildirimSheetsEnabled?.()) {
        ...
        if (!result.success) {
        (window.Utils ? window.Utils.showToast('Kayit hatasi: ' + (result.error || 'Bilinmeyen hata'), 'error') : console.error(result.error));
            return;
        }
...
async function handleListAction(event) {
    ...
    if (action === 'toggle') {
        ...
        if (window.setAnnouncementActiveOnSheets && window.isBildirimSheetsEnabled?.()) {
            ...
            if (!result.success) {
                (window.Utils ? window.Utils.showToast('Durum guncellenemedi: ' + (result.error || 'Bilinmeyen hata'), 'error') : console.error(result.error));
                return;
            }
    ...
    if (action === 'complete') {
        if (window.completeAnnouncementOnSheets && window.isBildirimSheetsEnabled?.()) {
            ...
            if (!result.success) {
                (window.Utils ? window.Utils.showToast('Bildirim tamamlanamadi: ' + (result.error || 'Bilinmeyen hata'), 'error') : console.error(result.error));
                return;
            }
    ...
    if (action === 'delete' && confirm('Bu bildirimi silmek istiyor musunuz?')) {
        if (window.deleteAnnouncementFromSheets && window.isBildirimSheetsEnabled?.()) {
            ...
            if (!result.success) {
                (window.Utils ? window.Utils.showToast('Bildirim silinemedi: ' + (result.error || 'Bilinmeyen hata'), 'error') : console.error(result.error));
                return;
            }
...
async function clearInactiveAnnouncements() {
    ...
    if (window.clearInactiveAnnouncementsOnSheets && window.isBildirimSheetsEnabled?.()) {
        ...
        if (!result.success) {
            (window.Utils ? window.Utils.showToast('Pasif bildirimler silinemedi: ' + (result.error || 'Bilinmeyen hata'), 'error') : console.error(result.error));
            return;
        }
...
function escapeHtml(value) {
    return window.Utils ? window.Utils.escapeHtml(value) : String(value || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;');
}
