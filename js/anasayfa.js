document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadLatestMotorStatus() {
        try {
            console.log('Motor durumu yükleniyor...');
            const url = new URL(KOJEN_MOTOR_APPS_SCRIPT_URL);
            url.searchParams.append('action', 'getLastRecords');
            url.searchParams.append('count', '100');

            console.log('Motor API URL:', url.toString());
            const response = await fetch(url, { method: 'GET', mode: 'cors' });
            const result = await response.json();

            console.log('Motor API yanıtı:', result);
    ...
    function showNotification(message, type = 'info') {
        ...
        // Modal açıldığında focus'u modal içine taşı (accessibility için)
        setTimeout(() => {
        ...
        // Focus'u modal dışına taşı (accessibility için)
        ...
        // Focus dışarı taşındıktan sonra aria-hidden'ı true yap
        setTimeout(() => {
...
setInterval(checkPreviousDaySteamData, 30 * 60 * 1000); // Her 30 dakikada bir kontrol
    ...
    // ========================================
    // PERİYODİK BAKIM VERİLERİ
    // ========================================
    
    // Periyodik bakım Apps Script URL'i
    const PERIYODIK_BAKIM_SCRIPT_URL = window.AppConfig.getScriptUrl('periyodikBakim');
    const PERIYODIK_BAKIM_CACHE_KEY = 'periyodikBakimData';
    const PERIYODIK_BAKIM_CACHE_TIME_KEY = 'periyodikBakimCacheTime';
    
    // Cache süresi: 5 dakika
    const PERIYODIK_BAKIM_CACHE_DURATION = 5 * 60 * 1000;
    
    // Periyodik bakım verileri
    const periyodikBakimData = {
        gm1: { bakimaKalanSaat: 0, greslemeKalanSaat: 0, numuneKalanSaat: 0 },
        gm2: { bakimaKalanSaat: 0, greslemeKalanSaat: 0, numuneKalanSaat: 0 },
        gm3: { bakimaKalanSaat: 0, greslemeKalanSaat: 0, numuneKalanSaat: 0 }
    };
    
    // Admin kullanıcı kontrolü ve bakım kartlarını göster
    function checkAndShowMaintenanceCards() {
        // getLoggedInUser fonksiyonu dosyanın daha aşağısında tanımlı, burada kullanılamıyor
        // localStorage'dan direkt okuyalım
        let user = null;
        try {
        ...
        const maintenanceSection = document.querySelector('.maintenance-status-section');
        
        if (maintenanceSection && user && user.role === 'admin') {
            maintenanceSection.classList.add('show-admin');
            maintenanceSection.style.display = 'block';
            
            // Cache'den yükle
            loadCachedPeriyodikBakimData();
            
            // Verileri güncelle
            loadPeriyodikBakimData();
            
            // Her 5 dakikada bir güncelle
            setInterval(loadPeriyodikBakimData, PERIYODIK_BAKIM_CACHE_DURATION);
        }
    }
    
    // Cache'den periyodik bakım verilerini yükle
    function loadCachedPeriyodikBakimData() {
        try {
            const cached = localStorage.getItem(PERIYODIK_BAKIM_CACHE_KEY);
            const cacheTime = localStorage.getItem(PERIYODIK_BAKIM_CACHE_TIME_KEY);
            
            if (cached && cacheTime) {
                const now = new Date().getTime();
                const cachedTime = parseInt(cacheTime, 10);
                
                if (now - cachedTime < PERIYODIK_BAKIM_CACHE_DURATION) {
                    const data = JSON.parse(cached);
                    Object.keys(periyodikBakimData).forEach(motor => {
                        if (data[motor]) {
                            periyodikBakimData[motor] = data[motor];
                        }
                    });
                    updatePeriyodikBakimDisplay();
                }
            }
        } catch (error) {
            console.error('Periyodik bakım cache okunamadı:', error);
        }
    }
    
    // Periyodik bakım verilerini Google Sheets'ten çek
    async function loadPeriyodikBakimData() {
        try {
            console.log('Periyodik bakım verileri yükleniyor...');
            
            // URL kontrolü
            if (!PERIYODIK_BAKIM_SCRIPT_URL || PERIYODIK_BAKIM_SCRIPT_URL === '') {
                console.warn('Periyodik bakım Apps Script URL tanımlanmamış. app-config.js dosyasına "periyodikBakim" URL ekleyin.');
                return;
            }
            
            const url = new URL(PERIYODIK_BAKIM_SCRIPT_URL);
            url.searchParams.append('action', 'getPeriyodikBakimData');
            ...
            const result = await response.json();
            console.log('Periyodik bakım API yanıtı:', result);
            
            if (result.success && result.data) {
                // Verileri güncelle
                Object.keys(periyodikBakimData).forEach(motor => {
                    if (result.data[motor]) {
                        periyodikBakimData[motor] = {
                            bakimaKalanSaat: parseFloat(result.data[motor].bakimaKalanSaat) || 0,
                            greslemeKalanSaat: parseFloat(result.data[motor].greslemeKalanSaat) || 0,
                            numuneKalanSaat: parseFloat(result.data[motor].numuneKalanSaat) || 0
                        };
                    }
                });
                
                // Cache'e kaydet
                localStorage.setItem(PERIYODIK_BAKIM_CACHE_KEY, JSON.stringify(periyodikBakimData));
                localStorage.setItem(PERIYODIK_BAKIM_CACHE_TIME_KEY, new Date().getTime().toString());
                
                // Ekranı güncelle
                updatePeriyodikBakimDisplay();
                
                console.log('Periyodik bakım verileri güncellendi:', periyodikBakimData);
            } else {
                console.error('Periyodik bakım verileri alınamadı:', result.error || result.message);
            }
        } catch (error) {
            console.error('Periyodik bakım verileri yüklenemedi:', error);
        }
    }
    
    // Periyodik bakım verilerini ekranda göster
    function updatePeriyodikBakimDisplay() {
        Object.keys(periyodikBakimData).forEach(motorId => {
            const data = periyodikBakimData[motorId];
            
            // Bakıma Kalan Saat
            const bakimEl = document.getElementById(`${motorId}-bakim-kalan`);
            if (bakimEl) {
                bakimEl.textContent = Math.round(data.bakimaKalanSaat) + ' saat';
                applyMaintenanceColorClass(bakimEl, data.bakimaKalanSaat);
            }
            
            // Greslemeye Kalan Saat
            const greslemeEl = document.getElementById(`${motorId}-gresleme-kalan`);
            if (greslemeEl) {
                greslemeEl.textContent = Math.round(data.greslemeKalanSaat) + ' saat';
                applyMaintenanceColorClass(greslemeEl, data.greslemeKalanSaat);
            }
            
            // Numuneye Kalan Saat
            const numuneEl = document.getElementById(`${motorId}-numune-kalan`);
            if (numuneEl) {
                numuneEl.textContent = Math.round(data.numuneKalanSaat) + ' saat';
                applyMaintenanceColorClass(numuneEl, data.numuneKalanSaat);
            }
        });
    }
    
    // Kalan saate göre renk sınıfı uygula
    function applyMaintenanceColorClass(element, hours) {
        // Tüm renk sınıflarını temizle
        element.classList.remove('warning', 'critical');
        
        // Saat değerine göre renk ekle
        if (hours < 50) {
            element.classList.add('critical'); // Kırmızı - Çok acil
        } else if (hours < 100) {
            element.classList.add('warning'); // Turuncu - Uyarı
        }
        // 100+ saat normal sarı renk olarak kalır
    }
    
    // Sayfa yüklendiğinde bakım kartlarını kontrol et
    setTimeout(checkAndShowMaintenanceCards, 500);


    // ========================================
    // MOTOR BAKIM DETAY MODAL
    // ========================================
    
    // Modal elementleri
    const motorDetailModal = document.getElementById('motorDetailModal');
    const motorDetailModalBody = document.getElementById('motorDetailModalBody');
    const motorDetailModalTitle = document.getElementById('motorDetailModalTitle');
    const closeMotorDetailBtns = document.querySelectorAll('[data-close-motor-detail]');
    
    // Bakım kartlarına click event ekle
    document.querySelectorAll('.maintenance-card-clickable').forEach(card => {
        card.addEventListener('click', function() {
            const motorId = this.getAttribute('data-motor');
            openMotorDetailModal(motorId);
        });
    });
    
    // Modal kapatma butonları
    closeMotorDetailBtns.forEach(btn => {
        btn.addEventListener('click', closeMotorDetailModal);
    });
    
    // Backdrop'a tıklayınca kapat
    if (motorDetailModal) {
        motorDetailModal.querySelector('.motor-detail-modal__backdrop')?.addEventListener('click', closeMotorDetailModal);
    }
    
    // ESC tuşu ile kapat
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape' && motorDetailModal?.classList.contains('is-open')) {
            closeMotorDetailModal();
        }
    });
    
    // Modal açma fonksiyonu
    async function openMotorDetailModal(motorId) {
        if (!motorDetailModal) return;
        
        // Modal'ı aç
        motorDetailModal.classList.add('is-open');
        motorDetailModal.setAttribute('aria-hidden', 'false');
        
        // Başlığı güncelle
        const motorName = motorId.toUpperCase().replace('GM', 'GM-');
        if (motorDetailModalTitle) {
            motorDetailModalTitle.textContent = `${motorName} Bakım Detayları`;
        }
        
        // Loading göster
        if (motorDetailModalBody) {
            motorDetailModalBody.innerHTML = '<div class="motor-detail-loading">Yükleniyor...</div>';
        }
        
        // Verileri çek
        try {
            const data = await fetchMotorDetailData(motorId);
            renderMotorDetailData(data);
        } catch (error) {
            console.error('Motor detay verileri yüklenemedi:', error);
            if (motorDetailModalBody) {
                motorDetailModalBody.innerHTML = `
                    <div class="motor-detail-loading" style="color: #e74c3c;">
    ...
    function closeMotorDetailModal() {
        if (!motorDetailModal) return;
        motorDetailModal.classList.remove('is-open');
        motorDetailModal.setAttribute('aria-hidden', 'true');
    }
    
    // Motor detay verilerini API'den çek
    async function fetchMotorDetailData(motorId) {
        const url = new URL(PERIYODIK_BAKIM_SCRIPT_URL);
        url.searchParams.append('action', 'getMotorDetailData');
        url.searchParams.append('motorId', motorId);
    ...
    // Motor detay verilerini render et
    function renderMotorDetailData(data) {
        if (!motorDetailModalBody) return;
        
        const html = `
            <!-- Güncel Çalışma Saati Bilgileri -->
            <div class="motor-detail-section">
                <h3 class="motor-detail-section-title">Güncel Çalışma Saati</h3>
                <div class="motor-info-box">
                    <div class="motor-info-item">
                        <div class="motor-info-label">Motor Saati</div>
                        <div class="motor-info-value highlight-value">${formatNumber(data.guncelCalismaSaati.saat)} saat</div>
                    </div>
                    <div class="motor-info-item">
                        <div class="motor-info-label">Kaynak</div>
                        <div class="motor-info-value" style="font-size: 14px;">${escapeHtml(data.guncelCalismaSaati.kaynak)}</div>
                    </div>
                    <div class="motor-info-item">
                        <div class="motor-info-label">Son Güncelleme</div>
                        <div class="motor-info-value" style="font-size: 14px;">${escapeHtml(data.guncelCalismaSaati.sonGuncelleme)}</div>
                    </div>
                </div>
                ${data.guncelCalismaSaati.not ? `<p style="margin-top: 12px; padding: 12px; background: #f8f9fa; border-radius: 8px; font-size: 13px; color: #64748b;"><strong>Not:</strong> ${escapeHtml(data.guncelCalismaSaati.not)}</p>` : ''}
            </div>
            
            <!-- Periyodik Bakım Bilgileri -->
            <div class="motor-detail-section">
                <h3 class="motor-detail-section-title">Periyodik Bakım Durumu</h3>
                <table class="motor-detail-table">
                    <thead>
                        <tr>
                            <th>Güncel Motor Saati</th>
                            <th>Son Periyodik Eşik</th>
                            <th>Son Bakım Tipi</th>
                            <th>Sonraki Periyodik Eşik</th>
                            <th>Sonraki Bakım Tipi</th>
                            <th>Bakıma Kalan Saat</th>
                            <th>Durum</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td class="highlight-value">${formatNumber(data.periyodikBakim.guncelMotorSaati)}</td>
                            <td>${formatNumber(data.periyodikBakim.sonPeriyodikEsik)}</td>
                            <td>${escapeHtml(data.periyodikBakim.sonBakimTipi)}</td>
                            <td>${formatNumber(data.periyodikBakim.sonrakiPeriyodikEsik)}</td>
                            <td>${escapeHtml(data.periyodikBakim.sonrakiBakimTipi)}</td>
                            <td class="highlight-value ${getStatusClass(data.periyodikBakim.bakimaKalanSaat)}">${formatNumber(data.periyodikBakim.bakimaKalanSaat)}</td>
                            <td><span class="status-badge ${getStatusClass(data.periyodikBakim.bakimaKalanSaat)}">${escapeHtml(data.periyodikBakim.durum)}</span></td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <!-- Gresleme Bilgileri -->
            <div class="motor-detail-section">
                <h3 class="motor-detail-section-title">Alternator Gresleme Durumu</h3>
                <table class="motor-detail-table">
                    <thead>
                        <tr>
                            <th>Son Alternatör Gresleme Saati</th>
                            <th>Güncel Motor Saati</th>
                            <th>Sonraki Gresleme Saati</th>
                            <th>Greslemeye Kalan Saat</th>
                            <th>Durum</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>${formatNumber(data.gresleme.sonAlternatorGreslemeSaati)}</td>
                            <td class="highlight-value">${formatNumber(data.gresleme.guncelMotorSaati)}</td>
                            <td>${formatNumber(data.gresleme.sonrakiGreslemeSaati)}</td>
                            <td class="highlight-value ${getStatusClass(data.gresleme.greslemeKalanSaat)}">${formatNumber(data.gresleme.greslemeKalanSaat)}</td>
                            <td><span class="status-badge ${getStatusClass(data.gresleme.greslemeKalanSaat)}">${escapeHtml(data.gresleme.durum)}</span></td>
                        </tr>
                    </tbody>
                </table>
            </div>
            
            <!-- Yağ Numune Bilgileri -->
            <div class="motor-detail-section">
                <h3 class="motor-detail-section-title">Yağ Numune Durumu</h3>
                <table class="motor-detail-table">
                    <thead>
                        <tr>
                            <th>Son Yağ Numune Saati</th>
                            <th>Güncel Motor Saati</th>
                            <th>Sonraki Numune Saati</th>
                            <th>Numuneye Kalan Saat</th>
                            <th>Durum</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>${formatNumber(data.yagNumune.sonYagNumuneSaati)}</td>
                            <td class="highlight-value">${formatNumber(data.yagNumune.guncelMotorSaati)}</td>
                            <td>${formatNumber(data.yagNumune.sonrakiNumuneSaati)}</td>
                            <td class="highlight-value ${getStatusClass(data.yagNumune.numuneKalanSaat)}">${formatNumber(data.yagNumune.numuneKalanSaat)}</td>
                            <td><span class="status-badge ${getStatusClass(data.yagNumune.numuneKalanSaat)}">${escapeHtml(data.yagNumune.durum)}</span></td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        motorDetailModalBody.innerHTML = html;
    }
