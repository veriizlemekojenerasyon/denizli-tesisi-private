...
document.addEventListener('DOMContentLoaded', function() {
    if (!checkAuth()) return;
    // Admin session token'ı GAS ile doğrula
    if (window.SessionAuth) {
        window.SessionAuth.validateAdminSession().then(function(valid) {
            if (!valid) return; // validateAdminSession zaten yönlendirir
...
function checkAuth() {
    ...
    try {
        ...
        const display = document.getElementById('adminUserName');
        if (display) {
...
function logout() {
    if (window.SessionAuth) {
        window.SessionAuth.clearSession(false);
    } else {
        localStorage.removeItem('loggedInUser');
...
async function loadUsers() {
    showLoading(true);
    try {
        const token = window.SessionAuth ? window.SessionAuth.getSessionToken() : '';
        const res = await fetch(USER_URL + '?action=getAllUsers&token=' + encodeURIComponent(token));
...
function addActorInfo(payload) {
    ...
    if (window.SessionAuth) {
        payload.sessionToken = window.SessionAuth.getSessionToken() || '';
    }
    return payload;
}
