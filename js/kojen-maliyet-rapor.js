...
async function handleBaglantiCek() {
  ...
  window[cbName] = function (json) {
    ...
    delete window[cbName];
    if (!json || !json.success) {
      var errMsg = json && json.tarihUyusmadi
        ? '⚠️ ' + (json.istenenTarih) + ' tarihi için veri yok.<br>Sayfada mevcut veri: <strong>' + json.mevcutTarih + '</strong><br><small>BaglantiNoktalari sayfası her gün üzerine yazılır, sadece son günün verisi tutulur.</small>'
        : 'Hata: ' + (json && json.error ? json.error : 'Sunucu hatası');
      document.getElementById('baglantiTableBody').innerHTML = '<tr><td colspan="7" class="empty-row" style="color:#c53030;line-height:1.6;">' + errMsg + '</td></tr>';
      setEl('baglantiTarihLabel','Bağlantı noktası verileri');
      return;
    }
    var rows = json.data || [];
    ...
    setEl('baglantiTarihLabel','Tarih: ' + (json.tarih || formatDateTR(tarih)));
  };
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  OtoHesap.init();
  KritikGun.init();
  // Sayfa yüklenince muaf günleri arka planda çek, özet bar'ı göster
  KritikGun.yukle();
});
...
var KritikGun = {
  ...
  overlay      : null,
  secilenGunler: [],   // ISO string array: ['2026-07-06', ...]
  teiasAktif   : true, // Toggle durumu

  // 2026 Temmuz-Eylül arası sabit resmi tatiller
  RESMI_TATILLER_2026: [
    { iso: '2026-07-15', ad: 'Demokrasi ve Milli Birlik Günü' },
    { iso: '2026-08-30', ad: 'Zafer Bayramı' }
  ],

  HAFTANIN_GUNLERI: ['Pazar', 'Pazartesi', 'Salı', 'Çarşamba', 'Perşembe', 'Cuma', 'Cumartesi'],

  init: function () {
    this.overlay = document.getElementById('kritikGunModal');
    if (!this.overlay) return;

    on('kritikGunBtn',      'click', function () { KritikGun.ac(); });
    on('kritikGunModalKapat','click', function () { KritikGun.kapat(); });
    on('muafIptalBtn',      'click', function () { KritikGun.kapat(); });
    on('muafHesaplaBtn',    'click', function () { KritikGun.hesaplaGunler(); });
    on('muafKaydetBtn',     'click', function () { KritikGun.kaydet(); });
    on('muafElleEkleBtn',   'click', function () { KritikGun.elleEkle(); });
    on('teiasToggleBtn',    'click', function () { KritikGun.toggleTeis(); });

    // Modal dışına tıkla → kapat
    this.overlay.addEventListener('click', function (e) {
      if (e.target === KritikGun.overlay) KritikGun.kapat();
    });
  },
  ...
  // GAS'tan kayıtlı muaf günleri yükle
  yukle: function () {
    var url = window.AppConfig ? window.AppConfig.getScriptUrl('kojenMaliyetRapor') : '';
    if (!url) return;

    fetch(url + '?action=getMuafGunler', { method: 'GET', mode: 'cors', cache: 'no-cache' })
      .then(function (r) { return r.json(); })
      .then(function (res) {
        if (res.success) {
          if (Array.isArray(res.data) && res.data.length) {
            KritikGun.secilenGunler = res.data.slice();
            KritikGun.listeGuncelle();
            KritikGun.noticeCal(res.data.length + ' kayıtlı muaf gün yüklendi.', 'success');
          }
          // Toggle durumunu yükle
          KritikGun.teiasAktif = res.teiasAktif !== false;
          KritikGun.toggleGuncelle();
        }
      })
      .catch(function () { /* bağlantı yoksa sessizce geç */ });
  },

  // Tarih aralığındaki Pazar ve resmi tatilleri hesapla
  hesaplaGunler: function () {
    var bas  = document.getElementById('muafBaslangic') && document.getElementById('muafBaslangic').value;
    var bit  = document.getElementById('muafBitis')     && document.getElementById('muafBitis').value;
    var pazarAktif  = document.getElementById('muafPazarOtomatik') && document.getElementById('muafPazarOtomatik').checked;
    var tatilAktif  = document.getElementById('muafTatilOtomatik') && document.getElementById('muafTatilOtomatik').checked;

    if (!bas || !bit) {
      this.noticeCal('Lütfen başlangıç ve bitiş tarihi seçin.', 'error');
      return;
    }

    var basTarih = new Date(bas + 'T00:00:00');
    var bitTarih = new Date(bit + 'T00:00:00');

    if (basTarih > bitTarih) {
      this.noticeCal('Başlangıç tarihi bitiş tarihinden sonra olamaz.', 'error');
      return;
    }

    var bulunan = [];
    var mevcut  = new Date(basTarih);

    while (mevcut <= bitTarih) {
      var iso = this._isoFormat(mevcut);

      if (pazarAktif && mevcut.getDay() === 0) {
        bulunan.push({ iso: iso, ad: 'Pazar' });
      }

      if (tatilAktif) {
        for (var t = 0; t < this.RESMI_TATILLER_2026.length; t++) {
          if (this.RESMI_TATILLER_2026[t].iso === iso) {
            bulunan.push({ iso: iso, ad: this.RESMI_TATILLER_2026[t].ad });
          }
        }
      }

      mevcut.setDate(mevcut.getDate() + 1);
    }

    // Mevcut seçimle birleştir, tekrar olmasın
    var yeniList = this.secilenGunler.slice();
    for (var b = 0; b < bulunan.length; b++) {
      if (yeniList.indexOf(bulunan[b].iso) === -1) {
        yeniList.push(bulunan[b].iso);
      }
    }
    yeniList.sort();
    this.secilenGunler = yeniList;
    this.listeGuncelle();
    this.noticeCal(bulunan.length + ' gün eklendi. Toplam ' + this.secilenGunler.length + ' muaf gün.', 'success');
  },

  // Elle gün ekle
  elleEkle: function () {
    var input = document.getElementById('muafElleEkle');
    if (!input || !input.value) {
      this.noticeCal('Lütfen bir tarih seçin.', 'error');
      return;
    }
    var iso = input.value; // YYYY-MM-DD
    if (this.secilenGunler.indexOf(iso) !== -1) {
      this.noticeCal('Bu gün zaten listede var.', 'error');
      return;
    }
    this.secilenGunler.push(iso);
    this.secilenGunler.sort();
    this.listeGuncelle();
    input.value = '';
    this.noticeCal('Gün eklendi.', 'success');
  },

  // GAS'a kaydet
  kaydet: function () {
    var btn = document.getElementById('muafKaydetBtn');
    if (btn) { btn.disabled = true; btn.textContent = 'Kaydediliyor...'; }

    var url  = window.AppConfig ? window.AppConfig.getScriptUrl('kojenMaliyetRapor') : '';
    var json = JSON.stringify(this.secilenGunler);

    fetch(url + '?action=saveMuafGunler&gunler=' + encodeURIComponent(json) + '&teiasAktif=' + this.teiasAktif, {
      ...
      .then(function (res) {
        ...
        if (res.success) {
          KritikGun.noticeCal(res.count + ' muaf gün GAS\'a kaydedildi. Bir sonraki hesaplamada uygulanacak.', 'success');
          setTimeout(function () { KritikGun.kapat(); }, 1000);
        } else {
          KritikGun.noticeCal('Hata: ' + (res.error || 'Kayıt başarısız'), 'error');
        }
      })
      .catch(function (err) {
        if (btn) { btn.disabled = false; btn.textContent = 'Kaydet ve Uygula'; }
        KritikGun.noticeCal('Bağlantı hatası: ' + err.message, 'error');
      });
  },

  // Listeden gün çıkar
  gunKaldir: function (iso) {
    var idx = this.secilenGunler.indexOf(iso);
    if (idx !== -1) this.secilenGunler.splice(idx, 1);
    this.listeGuncelle();
  },

  // Toggle aç/kapat
  toggleTeis: function () {
    this.teiasAktif = !this.teiasAktif;
    this.toggleGuncelle();
  },

  // Toggle görünümünü güncelle
  toggleGuncelle: function () {
    var btn   = document.getElementById('teiasToggleBtn');
    var knob  = document.getElementById('teiasToggleKnob');
    var acik  = document.getElementById('teiasToggleAciklama');

    if (btn) {
      btn.style.background = this.teiasAktif ? '#16a34a' : '#94a3b8';
      btn.setAttribute('aria-pressed', String(this.teiasAktif));
    }
    if (knob) {
      knob.style.left = this.teiasAktif ? '26px' : '3px';
    }
    if (acik) {
      acik.textContent = this.teiasAktif
        ? 'Aktif — TEİAŞ hesaplanıyor (muaf günler hariç)'
        : 'Kapalı — TEİAŞ hiçbir gün hesaplanmıyor';
      acik.style.color = this.teiasAktif ? '#64748b' : '#dc2626';
    }
    this.sayfaOzetiGuncelle();
  },

  // Sayfa üstündeki özet bar'ı güncelle
  sayfaOzetiGuncelle: function () {
    var bar   = document.getElementById('muafiyetOzetBar');
    var metin = document.getElementById('muafiyetOzetMetin');
    if (!bar || !metin) return;

    // Toggle kapalıysa kırmızı uyarı göster
    if (!this.teiasAktif) {
      bar.style.background   = '#fef2f2';
      bar.style.borderColor  = '#fecaca';
      bar.style.borderLeftColor = '#dc2626';
      bar.querySelector('svg').style.stroke = '#dc2626';
      bar.querySelector('span[style*="font-weight:700"]').style.color  = '#dc2626';
      bar.querySelector('span[style*="font-weight:700"]').textContent  = 'TEİAŞ Hesabı Kapalı';
      metin.style.color   = '#991b1b';
      metin.textContent   = 'Tüm günler için TEİAŞ = 0';
      bar.style.display   = 'flex';
      bar.hidden          = false;
      return;
    }

    // Toggle açık — muaf gün varsa yeşil özet
    bar.style.background      = '#f0fdf4';
    bar.style.borderColor     = '#bbf7d0';
    bar.style.borderLeftColor = '#16a34a';
    var svgEl = bar.querySelector('svg');
    if (svgEl) svgEl.style.stroke = '#16a34a';
    var baslikEl = bar.querySelector('span[style*="font-weight:700"]');
    if (baslikEl) { baslikEl.style.color = '#15803d'; baslikEl.textContent = 'TEİAŞ Muafiyeti Aktif'; }
    metin.style.color = '#166534';

    var liste = this.secilenGunler;
    if (!liste || liste.length === 0) {
      bar.hidden = true;
      return;
    }

    var goster = liste.slice(0, 5).map(function (iso) {
      return iso.split('-').reverse().slice(0, 2).join('.');
    });
    var fazla = liste.length > 5 ? ' +' + (liste.length - 5) + ' gün daha' : '';
    metin.textContent = liste.length + ' gün muaf: ' + goster.join(' · ') + fazla;
    bar.style.display = 'flex';
    bar.hidden = false;
  },

  // Gün listesini DOM'a render et
  listeGuncelle: function () {
    var konteyner = document.getElementById('muafGunListesi');
    var badge     = document.getElementById('muafToplamBadge');
    if (!konteyner) return;

    this.sayfaOzetiGuncelle();

    if (badge) badge.textContent = this.secilenGunler.length + ' gün';

    if (this.secilenGunler.length === 0) {
      konteyner.innerHTML = '<span style="color:var(--text-muted);font-size:13px;padding:8px;">Henüz muaf gün seçilmedi.</span>';
      return;
    }

    var html = '';
    for (var i = 0; i < this.secilenGunler.length; i++) {
      var iso = this.secilenGunler[i];
      var d   = new Date(iso + 'T00:00:00');
      var gun = this.HAFTANIN_GUNLERI[d.getDay()];
      var ad  = this._tatilAdiGetir(iso);
      var etiket = this._isoTR(iso) + ' ' + gun + (ad ? ' — ' + ad : '');
      var renk   = (d.getDay() === 0) ? '#fee2e2' : '#fef3c7';
      var yazı   = (d.getDay() === 0) ? '#991b1b' : '#92400e';

      html += '<span style="' +
        'display:inline-flex;align-items:center;gap:6px;' +
        'background:' + renk + ';color:' + yazı + ';' +
        'padding:4px 10px;border-radius:20px;font-size:12px;font-weight:600;' +
        '">' +
        '<span>' + etiket + '</span>' +
        '<button type="button" onclick="KritikGun.gunKaldir(\'' + iso + '\')" ' +
        'style="background:none;border:none;cursor:pointer;color:' + yazı + ';font-size:14px;line-height:1;padding:0;" ' +
        'title="Kaldır">×</button>' +
        '</span>';
    }
  ...
  _tatilAdiGetir: function (iso) {
    for (var t = 0; t < this.RESMI_TATILLER_2026.length; t++) {
      if (this.RESMI_TATILLER_2026[t].iso === iso) return this.RESMI_TATILLER_2026[t].ad;
    }
    return '';
  },

  noticeCal: function (msg, tip) {
    var el = document.getElementById('kritikGunNotice');
    if (!el) return;
    el.className  = 'km-notice ' + (tip || '');
    el.textContent = msg;
    el.hidden = false;
