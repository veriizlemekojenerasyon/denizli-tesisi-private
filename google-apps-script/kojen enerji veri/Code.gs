...
// ─── MIRROR AYARLARI ────────────────────────────────────────────────────────
// Yedek (mirror) spreadsheet ID — hedef: Enerji Mirror GM-1 / GM-2 / GM-3 sayfaları
var KOJEN_ENERJI_MIRROR_SPREADSHEET_ID = '1sOJPeIT8R812sf8bPfFW3g2ternvaT1JMhPioojKDjk';
// Mirror sheet adı şablonu: 'Enerji Mirror GM-1', 'Enerji Mirror GM-2', 'Enerji Mirror GM-3'
function getEnerjiMirrorSheetName(motor) {
  return 'Enerji Mirror ' + normalizeEnerjiMotorLabel(motor);
}
// ────────────────────────────────────────────────────────────────────────────
...
function addRecord(data) {
  try {
    ...
    // 🔁 MIRROR — yeni kayıt mirror sheet'e de yazılır (hata ana kaydı etkilemez)
    try {
      enerjiMirrorAddRecord(motor, values);
    } catch (mirrorError) {
      Logger.log('Enerji mirror yazma hatasi: ' + mirrorError.toString());
      if (!result.warnings) result.warnings = [];
      result.warnings.push('Mirror yazma hatasi: ' + mirrorError.toString());
    }
    ...
    Object.keys(groupedRecords).forEach(function(motor) {
      try {
        ...
        if (rowsToAdd.length) {
          ...
          // 🔁 MIRROR — bu motor için yazılan satırları mirror sheet'e de gönder
          try {
            enerjiMirrorAddMultipleRecords(motor, rowsToAdd);
          } catch (mirrorError) {
            Logger.log('Enerji mirror toplu kayit hatasi (' + motor + '): ' + mirrorError.toString());
            errors.push({ motor: motor + '_mirror', error: mirrorError.toString() });
          }
        }
      } catch (recordError) {
        errors.push({
...
function installHourlyMissingRecordTrigger() {
  ...
  ScriptApp.newTrigger('checkHourlyMissingRecords')
    .timeBased()
    .everyHours(1)
    .atMinute(10) // Her saat 10. dakikada çalış (08:10, 09:10, 10:10...)
    .create();

  return { success: true, message: 'Enerji saatlik eksik kayit tetikleyicisi kuruldu (10. dakika). Kontrol saatte bir calisir; mail gunluk rapora ertelendi.' };
}
...
function updateRecord(params) {
  ...
  try {
    ...
    // 🔁 MIRROR — güncelleme mirror sheet'e de yansıtılır (hata ana işlemi etkilemez)
    try {
      enerjiMirrorUpdateRecord(motor, newValues);
    } catch (mirrorError) {
      Logger.log('Enerji mirror guncelleme hatasi (' + motor + '): ' + mirrorError.toString());
    }
...
// ═══════════════════════════════════════════════════════════════════════════════
// 🔁 MIRROR FONKSİYONLARI — Enerji Mirror GM-1 / GM-2 / GM-3
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Hedef spreadsheet'te 'Enerji Mirror GM-X' sayfasını getirir ya da oluşturur.
 * @param {string} motor  normalizeEnerjiMotorLabel ile normalleştirilmiş motor adı
 * @returns {Sheet}
 */
function getOrCreateEnerjiMirrorSheet(motor) {
  var ss = SpreadsheetApp.openById(KOJEN_ENERJI_MIRROR_SPREADSHEET_ID);
  var sheetName = getEnerjiMirrorSheetName(motor);
  ...
  var headers = [
    'Tarih', 'Vardiya', 'Saat', 'Motor',
    'AYDEM VOLTAJI', 'AKTİF GÜÇ', 'REAKTİF GÜÇ', 'Cos φ',
    'ORT.AKIM', 'ORT.GERİLİM', 'NÖTR AKIMI', 'TAHRİK GERİLİMİ',
    'TOPLAM AKTİF ENERJİ', 'ÇALIŞMA SAATİ', 'KALKIŞ SAYISI',
    'Durum', 'Kaydeden', 'Kayıt Tarihi'
  ];
  ...
  // Format ayarları
  sheet.getRange(2, 1, 2000, 4).setNumberFormat('@');   // Tarih, Vardiya, Saat, Motor
  sheet.getRange(2, 5, 2000, 11).setNumberFormat('0.00'); // Sayısal değerler
  sheet.getRange(2, 16, 2000, 3).setNumberFormat('@');  // Durum, Kaydeden, Kayıt Tarihi

  Logger.log('Enerji Mirror sayfa olusturuldu: ' + sheetName);
  return sheet;
}

/**
 * Mirror sheet'te belirli tarih+saat+motor satırını bulur.
 * @returns {number} Satır numarası (1-tabanlı), bulunamazsa -1
 */
function findEnerjiMirrorRow(sheet, tarih, saat, motor) {
  var lastRow = sheet.getLastRow();
  if (lastRow < 2) return -1;

  var data = sheet.getRange(2, 1, lastRow - 1, 4).getDisplayValues();
  var normTarih = normalizeDateTR(tarih);
  var normSaat  = normalizeEnerjiSaat(saat);
  var normMotor = normalizeEnerjiMotorLabel(motor);

  for (var i = 0; i < data.length; i++) {
    if (
      normalizeDateTR(data[i][0])           === normTarih &&
      normalizeEnerjiSaat(data[i][2])       === normSaat  &&
      normalizeEnerjiMotorLabel(data[i][3]) === normMotor
    ) {
      return i + 2;
    }
  }
...
/**
 * Yeni kayıt eklendiğinde mirror sheet'e de yazar.
 * Aynı tarih+saat+motor zaten varsa üzerine yazar (idempotent).
 * @param {string} motor
 * @param {Array}  values  18 elemanlı satır dizisi (addRecord ile aynı sıra)
 */
function enerjiMirrorAddRecord(motor, values) {
  try {
    var sheet = getOrCreateEnerjiMirrorSheet(motor);
    var tarih = values[0];
    var saat  = values[2];

    var existingRow = findEnerjiMirrorRow(sheet, tarih, saat, motor);
    if (existingRow !== -1) {
      sheet.getRange(existingRow, 1, 1, 18).setValues([values]);
      Logger.log('Enerji Mirror guncellendi (add): ' + motor + ' ' + tarih + ' ' + saat);
    } else {
      var lastRow = sheet.getLastRow();
      sheet.getRange(lastRow + 1, 1, 1, 18).setValues([values]);
      Logger.log('Enerji Mirror eklendi: ' + motor + ' ' + tarih + ' ' + saat);
    }

    var targetRow = existingRow !== -1 ? existingRow : sheet.getLastRow();
    ...
    sheet.getRange(targetRow, 5, 1, 11).setNumberFormat('0.00');

    if (String(values[15] || '').toUpperCase().indexOf('ÇALIŞMIYOR') !== -1 ||
        String(values[15] || '').toUpperCase().indexOf('CALISMIYOR') !== -1) {
      range.setFontColor('#c62828');
    }

    return { success: true };
  } catch (e) {
    Logger.log('enerjiMirrorAddRecord hatasi (' + motor + '): ' + e.toString());
    return { success: false, error: e.toString() };
  }
}

/**
 * Kayıt güncellendiğinde mirror sheet'teki ilgili satırı da günceller.
 * Satır bulunamazsa yeni satır ekler.
 * @param {string} motor
 * @param {Array}  newValues  18 elemanlı satır dizisi
 */
function enerjiMirrorUpdateRecord(motor, newValues) {
  try {
    var sheet = getOrCreateEnerjiMirrorSheet(motor);
    var tarih = newValues[0];
    var saat  = newValues[2];

    var targetRow = findEnerjiMirrorRow(sheet, tarih, saat, motor);
    if (targetRow === -1) {
      targetRow = sheet.getLastRow() + 1;
      Logger.log('Enerji Mirror satiri bulunamadi, yeni satir olarak eklendi: ' + motor + ' ' + tarih + ' ' + saat);
    }

    sheet.getRange(targetRow, 1, 1, 18).setValues([newValues]);
    ...
    if (String(newValues[15] || '').toUpperCase().indexOf('ÇALIŞMIYOR') !== -1 ||
        String(newValues[15] || '').toUpperCase().indexOf('CALISMIYOR') !== -1) {
      range.setFontColor('#c62828');
    }

    Logger.log('Enerji Mirror guncellendi (update): ' + motor + ' ' + tarih + ' ' + saat);
    return { success: true };
  } catch (e) {
    Logger.log('enerjiMirrorUpdateRecord hatasi (' + motor + '): ' + e.toString());
    return { success: false, error: e.toString() };
  }
}

/**
 * Çoklu kayıt eklemede mirror sheet'e toplu yazar.
 * @param {string}   motor
 * @param {Array[]}  rowsArray  Her elemanı 18 elemanlı dizi olan array
 */
function enerjiMirrorAddMultipleRecords(motor, rowsArray) {
  if (!rowsArray || rowsArray.length === 0) return { success: true, count: 0 };
  try {
    var sheet = getOrCreateEnerjiMirrorSheet(motor);
    var lastRow = sheet.getLastRow();
    ...
    if (lastRow >= 2) {
      ...
      for (var i = 0; i < metaRows.length; i++) {
        var k = normalizeDateTR(metaRows[i][0]) + '|' + normalizeEnerjiSaat(metaRows[i][2]);
        existingKeys[k] = i + 2;
      }
    }
    ...
    for (var j = 0; j < rowsArray.length; j++) {
      var row = rowsArray[j];
      var key = normalizeDateTR(row[0]) + '|' + normalizeEnerjiSaat(row[2]);
      if (existingKeys[key] !== undefined) {
        sheet.getRange(existingKeys[key], 1, 1, 18).setValues([row]);
      } else {
        toAppend.push(row);
        existingKeys[key] = lastRow + toAppend.length;
      }
    }

    if (toAppend.length > 0) {
      var appendStart = sheet.getLastRow() + 1;
      var appendRange = sheet.getRange(appendStart, 1, toAppend.length, 18);
      appendRange.setValues(toAppend);
      appendRange.setHorizontalAlignment('center');
      appendRange.setFontSize(10);
      appendRange.setBorder(true, true, true, true, true, true, '#cccccc', SpreadsheetApp.BorderStyle.SOLID);
      sheet.getRange(appendStart, 5, toAppend.length, 11).setNumberFormat('0.00');
    }

    Logger.log('enerjiMirrorAddMultiple: ' + motor + ' -> ' + rowsArray.length + ' kayit islendi (' + toAppend.length + ' yeni, ' + (rowsArray.length - toAppend.length) + ' guncelleme)');
    return { success: true, count: rowsArray.length, appended: toAppend.length };
  } catch (e) {
    Logger.log('enerjiMirrorAddMultiple hatasi (' + motor + '): ' + e.toString());
    return { success: false, error: e.toString() };
  }
}
