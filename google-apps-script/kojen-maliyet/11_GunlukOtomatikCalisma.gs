...
function gunlukVerileriCek(isoTarih, gun, ay, yil) {
  try {
    ...
    var gunHesap  = _gocSaatlikHesapla(ss, gun, ay, yil, bagSonuc, amrSonuc, teiasMuafMi(gun, ay, yil));
    ...
    try {
      ...
      if (fdSheet) {
        ...
        // Şebeke ve üretim değerleri — metin olarak yaz, nokta ondalık ayraç
        var sebeketxt = sebeke.toFixed(3);   // JS her zaman nokta kullanır
        var birimtxt  = birimMal.toFixed(5);
        var uretimtxt = uretim.toFixed(3);

        fdSheet.getRange(fdSatir, 12).setNumberFormat('@').setValue(sebeketxt)
          .setBackground('#EBF8EE').setFontWeight('bold')
          .setNote('Şebeke Tüketim\nTarih: ' + gun + '.' + ayKisa);
        fdSheet.getRange(fdSatir, 13).setNumberFormat('@').setValue(birimtxt)
          .setBackground('#FFF9C4').setFontWeight('bold')
          .setNote('Birim Maliyet = K' + fdSatir + ' / L' + fdSatir + '\n= ' + d28Val.toFixed(2) + ' ÷ ' + sebeketxt + '\n= D28 Toplam Maliyet (TL) ÷ H28 Şebeke Tüketim (MWh)');
        fdSheet.getRange(fdSatir, 16).setNumberFormat('@').setValue(uretimtxt)
          .setBackground('#EBF8EE').setFontWeight('bold')
        ...
        // Toplam satırını güncelle (L ve P metin formatında olduğundan GAS'ta topla)
        var gunSayisi   = new Date(yil, ay, 0).getDate();
        var topSatir    = gunSayisi + 2;

        // K toplamı — sayısal, SUM formülü yeterli
        fdSheet.getRange(topSatir, 11)
          .setFormula('=SUM(K2:K' + (gunSayisi + 1) + ')')
          .setBackground('#2c5282').setFontColor('#FFFFFF').setFontWeight('bold')
          .setNumberFormat('#,##0.00 "₺"');
        SpreadsheetApp.flush();

        // L toplamı — metin hücreler, GAS'ta topla
        var lRows = fdSheet.getRange(2, 12, gunSayisi, 1).getValues();
        var lToplam = 0;
        for (var li = 0; li < lRows.length; li++) {
          lToplam += parseFloat(String(lRows[li][0]).replace(',', '.')) || 0;
        }
        fdSheet.getRange(topSatir, 12)
          .setNumberFormat('@').setValue(lToplam.toFixed(3))
          .setBackground('#2c5282').setFontColor('#FFFFFF').setFontWeight('bold');

        // M toplam = K toplam / L toplam
        var kToplam = parseFloat(fdSheet.getRange(topSatir, 11).getValue()) || 0;
        var mToplam = lToplam > 0 ? kToplam / lToplam : 0;
        fdSheet.getRange(topSatir, 13)
          .setNumberFormat('@').setValue(mToplam.toFixed(5))
          .setBackground('#b7950b').setFontColor('#FFFFFF').setFontWeight('bold')
          .setNote('=K' + topSatir + '/L' + topSatir + '\n= Aylık Toplam Maliyet ÷ Aylık Şebeke Tüketim');

        // P toplamı — metin hücreler, GAS'ta topla
        var pRows = fdSheet.getRange(2, 16, gunSayisi, 1).getValues();
        var pToplam = 0;
        for (var pi = 0; pi < pRows.length; pi++) {
          pToplam += parseFloat(String(pRows[pi][0]).replace(',', '.')) || 0;
        }
        fdSheet.getRange(topSatir, 16)
          .setNumberFormat('@').setValue(pToplam.toFixed(3))
          .setBackground('#276749').setFontColor('#FFFFFF').setFontWeight('bold');

        SpreadsheetApp.flush();
        Logger.log('  FD : OK D28=' + d28Val.toFixed(2) + ' Şebeke=' + sebeke.toFixed(3) + ' Üretim=' + uretim + ' L33=' + lToplam.toFixed(3) + ' M33=' + mToplam.toFixed(5));
      } else {
 ...
 * }}
 */
function _gocSaatlikHesapla(ss, gun, ay, yil, bagSonuc, amrSonuc, teiasMuaf) {
  ...
  try {
    ...
    for (var i = 0; i < 24; i++) {
      ...
      // Muaf günlerde (Pazar + resmi tatil) TEİAŞ = 0
      var teias = 0;
      if (!teiasMuaf && i >= 16 && i <= 22) {
...
function doGet(e) {
  ...
  else if (action === 'getMuafGunler')        sonuc = getMuafGunler();
  else if (action === 'saveMuafGunler')       sonuc = saveMuafGunler(params);
...
function getBaglantiNoktalari(params) {
  try {
    ...
    if (!sheet || sheet.getLastRow() < 25) {
      return { success: false, error: 'BaglantiNoktalari sayfası bulunamadı veya boş.' };
    }

    // Sayfadaki mevcut veri tarihini oku
    var mevcutTarih = cfgBaglantiTarihiOku(ss);
    var mevcutTarihStr = cfgPad2(mevcutTarih.getDate()) + '.' +
                        cfgPad2(mevcutTarih.getMonth() + 1) + '.' +
                        mevcutTarih.getFullYear();

    // İstenen tarih ile karşılaştır
    var istenenTarih = String(params.date || '').trim();
    if (istenenTarih) {
      // YYYY-MM-DD formatını DD.MM.YYYY'ye çevir
      var parcalar = istenenTarih.split('-');
      var istenenTarihStr = parcalar.length === 3
        ? parcalar[2] + '.' + parcalar[1] + '.' + parcalar[0]
        : istenenTarih;

      if (istenenTarihStr !== mevcutTarihStr) {
        return {
          success      : false,
          tarihUyusmadi: true,
          mevcutTarih  : mevcutTarihStr,
          istenenTarih : istenenTarihStr,
          error        : 'İstenen tarih (' + istenenTarihStr + ') için veri yok. ' +
                        'Sayfada mevcut veri: ' + mevcutTarihStr
        };
      }
    }
    ...
    var rows  = batch.map(function(r, i) {
      return {
        saat    : cfgPad2(i) + ':00:00',
    ...
    return { success: true, data: rows, tarih: mevcutTarihStr };
  } catch(e) {
    return { success: false, error: e.toString() };
  }
}
...
// ─── TEİAŞ MUAF GÜN SİSTEMİ ─────────────────────────────────────────────────

var MUAF_GUNLER_KEY  = 'teiasMuafGunler';
var TEIAS_AKTIF_KEY  = 'teiasAktif';  // 'true' veya 'false'

/**
 * TEİAŞ hesabı genel olarak aktif mi?
 * Toggle kapalıysa tüm günler için TEİAŞ = 0.
 */
function teiasAktifMi() {
  try {
    var val = PropertiesService.getScriptProperties().getProperty(TEIAS_AKTIF_KEY);
    // Varsayılan: aktif (kayıt yoksa true)
...
/**
 * Verilen gün TEİAŞ hesabından muaf mı?
 * Toggle kapalıysa her gün muaf sayılır.
 * Toggle açıksa muaf gün listesine bakılır.
 */
function teiasMuafMi(gun, ay, yil) {
  try {
    // Toggle kapalıysa tüm günler muaf
    if (!teiasAktifMi()) return true;

    var iso = yil + '-' + cfgPad2(ay) + '-' + cfgPad2(gun);
    var raw = PropertiesService.getScriptProperties().getProperty(MUAF_GUNLER_KEY);
    if (!raw) return false;
    var liste = JSON.parse(raw);
    return Array.isArray(liste) && liste.indexOf(iso) !== -1;
  } catch (e) {
    Logger.log('teiasMuafMi hatasi: ' + e.toString());
    return false;
  }
}

/**
 * Kayıtlı muaf gün listesini ve toggle durumunu döndürür.
 */
function getMuafGunler() {
  try {
    var raw   = PropertiesService.getScriptProperties().getProperty(MUAF_GUNLER_KEY);
...
/**
 * Muaf gün listesini ve toggle durumunu kaydeder.
 * params.gunler    : JSON array string — ['2026-07-05', ...]
 * params.teiasAktif: 'true' veya 'false'
 */
function saveMuafGunler(params) {
  try {
    var raw = String(params.gunler || '[]').trim();
    var liste = JSON.parse(raw);
    if (!Array.isArray(liste)) {
      return { success: false, error: 'Geçersiz format: dizi bekleniyor.' };
    }
    var temiz = liste.filter(function(g) {
      return /^\d{4}-\d{2}-\d{2}$/.test(String(g));
    });

    // Toggle durumunu kaydet
    var aktif = String(params.teiasAktif || 'true').trim().toLowerCase();
    PropertiesService.getScriptProperties().setProperty(TEIAS_AKTIF_KEY, aktif === 'false' ? 'false' : 'true');
    PropertiesService.getScriptProperties().setProperty(MUAF_GUNLER_KEY, JSON.stringify(temiz));
    Logger.log('TEİAŞ muaf günler kaydedildi: ' + temiz.length + ' gün, aktif=' + aktif);
    return { success: true, count: temiz.length, data: temiz, teiasAktif: aktif !== 'false' };
  } catch (e) {
    return { success: false, error: e.toString() };
  }
}
