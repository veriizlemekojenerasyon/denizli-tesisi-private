...
function _fdAylikTablolariYaz(sheet, ay, yil, hesaplananGun) {
  ...
  for (var g = 1; g <= gunSayisi; g++) {
    ...
    if (buGun) {
      ...
      lHucre.setNumberFormat('@').setValue(h26Val.toFixed(3)).setBackground('#EBF8EE').setFontWeight('bold');
      mHucre.setNumberFormat('@').setValue(h26Val > 0 ? (d28Val / h26Val).toFixed(5) : '0.00000').setBackground('#FFF9C4').setFontWeight('bold');
    } else {
    ...
    if (buGun) {
      ...
      pHucre.setNumberFormat('@').setValue(uretimMwh.toFixed(3)).setBackground('#EBF8EE').setFontWeight('bold');
      SpreadsheetApp.flush();
  ...
  // Toplam satırları — L ve P metin formatında olduğundan GAS'ta topla, metin olarak yaz
  ...
  // L sütunu metin formatında — GAS'ta topla
  var lTopToplam = 0;
  var lVeriler = sheet.getRange(2, 12, gunSayisi, 1).getValues();
  for (var li = 0; li < lVeriler.length; li++) {
    var lVal = parseFloat(String(lVeriler[li][0]).replace(',', '.')) || 0;
    lTopToplam += lVal;
  }
  sheet.getRange(topSatir, 12).setNumberFormat('@').setValue(lTopToplam.toFixed(3))
    .setBackground('#2c5282').setFontColor('#FFFFFF').setFontWeight('bold');

  SpreadsheetApp.flush();
  var k33Val = parseFloat(sheet.getRange(topSatir, 11).getValue()) || 0;
  var m33Val = lTopToplam > 0 ? k33Val / lTopToplam : 0;

  // M33
  var k33Hucre = 'K' + topSatir;
  var l33Hucre = 'L' + topSatir;
  sheet.getRange(topSatir, 13)
    .setNumberFormat('@').setValue(m33Val.toFixed(5))
    .setBackground('#b7950b')
    .setFontColor('#FFFFFF')
    .setFontWeight('bold')
    .setNote('Formül: =' + k33Hucre + '/' + l33Hucre + '\n= Aylık Toplam Maliyet (TL) ÷ Aylık Şebeke Tüketim (MWh)');
  ...
  // P sütunu metin formatında — GAS'ta topla
  var pTopToplam = 0;
  var pVeriler = sheet.getRange(2, 16, gunSayisi, 1).getValues();
  for (var pi = 0; pi < pVeriler.length; pi++) {
    var pVal = parseFloat(String(pVeriler[pi][0]).replace(',', '.')) || 0;
    pTopToplam += pVal;
  }
  sheet.getRange(topSatir, 16).setNumberFormat('@').setValue(pTopToplam.toFixed(3))
    .setBackground('#276749').setFontColor('#FFFFFF').setFontWeight('bold');
