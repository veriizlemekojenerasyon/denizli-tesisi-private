...
var KOJEN_MOTOR_WRITE_LOCK_WAIT_MS = 30000;
...
// ─── MIRROR AYARLARI ────────────────────────────────────────────────────────
// Yedek (mirror) spreadsheet ID — hedef: Motor Mirror GM-1 / GM-2 / GM-3 sayfaları
var KOJEN_MOTOR_MIRROR_SPREADSHEET_ID = '1sOJPeIT8R812sf8bPfFW3g2ternvaT1JMhPioojKDjk';
// Mirror sheet adı şablonu: 'Motor Mirror GM-1', 'Motor Mirror GM-2', 'Motor Mirror GM-3'
function getMirrorSheetName(motor) {
  return 'Motor Mirror ' + normalizeMotorLabel(motor);
}
// ────────────────────────────────────────────────────────────────────────────
...
function addRecord(data) {
  ...
  try {
    ...
    var mirrorWarning = '';
    try {
      mirrorAddRecord(motor, values);
    } catch (mirrorError) {
      mirrorWarning = 'Mirror yazma hatasi: ' + mirrorError.toString();
      Logger.log(mirrorWarning);
      postWriteWarnings.push(mirrorWarning);
    }
    ...
    Object.keys(groupedRecords).forEach(function(motor) {
      try {
        ...
        if (rowsToAdd.length) {
          ...
          try {
            mirrorAddMultipleRecords(motor, rowsToAdd);
          } catch (mirrorError) {
            Logger.log('Mirror toplu kayit hatasi (' + motor + '): ' + mirrorError.toString());
...
function updateRecord(params) {
  ...
  try {
    ...
    try {
      mirrorUpdateRecord(motor, newValues);
    } catch (mirrorError) {
      Logger.log('Mirror guncelleme hatasi (' + motor + '): ' + mirrorError.toString());
    }
...
// ═══════════════════════════════════════════════════════════════════════════════
// 🔁 MIRROR FONKSİYONLARI — Motor Mirror GM-1 / GM-2 / GM-3
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * Hedef spreadsheet'te 'Motor Mirror GM-X' sayfasını getirir ya da oluşturur.
 * @param {string} motor  normalizeMotorLabel ile normalleştirilmiş motor adı (GM-1 vb.)
 * @returns {Sheet}
 */
function getOrCreateMirrorSheet(motor) {
  var ss = SpreadsheetApp.openById(KOJEN_MOTOR_MIRROR_SPREADSHEET_ID);
  var sheetName = getMirrorSheetName(motor);
  ...
  // Metin formatları
  sheet.getRange(2, 1, 2000, 1).setNumberFormat('@');  // Tarih
  sheet.getRange(2, 2, 2000, 1).setNumberFormat('@');  // Vardiya
  sheet.getRange(2, 3, 2000, 1).setNumberFormat('@');  // Saat
  sheet.getRange(2, 4, 2000, 1).setNumberFormat('@');  // Motor
  sheet.getRange(2, 5, 2000, 15).setNumberFormat('0.00'); // Sayısal değerler
  sheet.getRange(2, 20, 2000, 3).setNumberFormat('@'); // Durum, Kaydeden, Kayıt Tarihi

  Logger.log('Mirror sayfa olusturuldu: ' + sheetName);
  return sheet;
}
 ...
 */
function findMirrorRow(sheet, tarih, saat, motor) {
  var lastRow = sheet.getLastRow();
  if (lastRow < 2) return -1;

  var data = sheet.getRange(2, 1, lastRow - 1, 4).getDisplayValues();
  var normTarih = normalizeDateTR(tarih);
  var normSaat  = normalizeMotorSaat(saat);
  var normMotor = normalizeMotorLabel(motor);

  for (var i = 0; i < data.length; i++) {
    if (
      normalizeDateTR(data[i][0])      === normTarih &&
      normalizeMotorSaat(data[i][2])   === normSaat  &&
      normalizeMotorLabel(data[i][3])  === normMotor
    ) {
 ...
 * @param {string} motor
 * @param {Array}  values  22 elemanlı satır dizisi (addRecord ile aynı sıra)
 */
function mirrorAddRecord(motor, values) {
  try {
    var sheet = getOrCreateMirrorSheet(motor);
    var tarih = values[0];
    var saat  = values[2];

    var existingRow = findMirrorRow(sheet, tarih, saat, motor);
    if (existingRow !== -1) {
      // Zaten var — güncelle
      sheet.getRange(existingRow, 1, 1, 22).setValues([values]);
      Logger.log('Mirror guncellendi (add): ' + motor + ' ' + tarih + ' ' + saat);
    } else {
      // Yeni satır ekle
      var lastRow = sheet.getLastRow();
      sheet.getRange(lastRow + 1, 1, 1, 22).setValues([values]);
      Logger.log('Mirror eklendi: ' + motor + ' ' + tarih + ' ' + saat);
    }

    // Basit formatlama
    var targetRow = existingRow !== -1 ? existingRow : sheet.getLastRow();
    var range = sheet.getRange(targetRow, 1, 1, 22);
    range.setHorizontalAlignment('center');
    range.setFontSize(10);
    range.setBorder(true, true, true, true, true, true, '#cccccc', SpreadsheetApp.BorderStyle.SOLID);
    sheet.getRange(targetRow, 5, 1, 15).setNumberFormat('0.00');

    if (String(values[19] || '').toUpperCase().indexOf('ÇALIŞMIYOR') !== -1 ||
        String(values[19] || '').toUpperCase().indexOf('CALISMIYOR') !== -1) {
      range.setFontColor('#c62828');
    }

    return { success: true };
  } catch (e) {
    Logger.log('mirrorAddRecord hatasi (' + motor + '): ' + e.toString());
 ...
 * @param {string} motor
 * @param {Array}  newValues  22 elemanlı satır dizisi
 */
function mirrorUpdateRecord(motor, newValues) {
  try {
    var sheet = getOrCreateMirrorSheet(motor);
    var tarih = newValues[0];
    var saat  = newValues[2];

    var targetRow = findMirrorRow(sheet, tarih, saat, motor);
    if (targetRow === -1) {
      // Mirror'da yok — ekle
      var lastRow = sheet.getLastRow();
      targetRow = lastRow + 1;
      Logger.log('Mirror satirı bulunamadi, yeni satir olarak eklendi: ' + motor + ' ' + tarih + ' ' + saat);
    }

    sheet.getRange(targetRow, 1, 1, 22).setValues([newValues]);
    ...
    if (String(newValues[19] || '').toUpperCase().indexOf('ÇALIŞMIYOR') !== -1 ||
        String(newValues[19] || '').toUpperCase().indexOf('CALISMIYOR') !== -1) {
      range.setFontColor('#c62828');
    }

    Logger.log('Mirror guncellendi (update): ' + motor + ' ' + tarih + ' ' + saat);
    return { success: true };
  } catch (e) {
    Logger.log('mirrorUpdateRecord hatasi (' + motor + '): ' + e.toString());
    return { success: false, error: e.toString() };
  }
}
 ...
 * @param {string} motor
 * @param {Array[]}  rowsArray  Her elemanı 22 elemanlı dizi olan array
 */
function mirrorAddMultipleRecords(motor, rowsArray) {
  if (!rowsArray || rowsArray.length === 0) return { success: true, count: 0 };
  try {
    var sheet = getOrCreateMirrorSheet(motor);
    var lastRow = sheet.getLastRow();

    // Mevcut mirror kayıtlarının tarih+saat anahtarlarını al
    var existingKeys = {};
    if (lastRow >= 2) {
      var metaRows = sheet.getRange(2, 1, lastRow - 1, 3).getDisplayValues();
      for (var i = 0; i < metaRows.length; i++) {
        var k = normalizeDateTR(metaRows[i][0]) + '|' + normalizeMotorSaat(metaRows[i][2]);
        existingKeys[k] = i + 2; // satır numarası
      }
    }
    ...
    for (var j = 0; j < rowsArray.length; j++) {
      var row = rowsArray[j];
      var key = normalizeDateTR(row[0]) + '|' + normalizeMotorSaat(row[2]);
      if (existingKeys[key] !== undefined) {
        // Zaten var — satırı güncelle
        sheet.getRange(existingKeys[key], 1, 1, 22).setValues([row]);
      } else {
        toAppend.push(row);
        existingKeys[key] = lastRow + toAppend.length; // ileriki satır numarasını kaydet
      }
    }

    if (toAppend.length > 0) {
      var appendStart = sheet.getLastRow() + 1;
      var appendRange = sheet.getRange(appendStart, 1, toAppend.length, 22);
      ...
      sheet.getRange(appendStart, 5, toAppend.length, 15).setNumberFormat('0.00');
    }

    Logger.log('mirrorAddMultipleRecords: ' + motor + ' -> ' + rowsArray.length + ' kayit islendi (' + toAppend.length + ' yeni, ' + (rowsArray.length - toAppend.length) + ' guncelleme)');
    return { success: true, count: rowsArray.length, appended: toAppend.length };
  } catch (e) {
    Logger.log('mirrorAddMultipleRecords hatasi (' + motor + '): ' + e.toString());
    return { success: false, error: e.toString() };
  }
}
