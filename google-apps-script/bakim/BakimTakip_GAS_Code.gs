...
const SPREADSHEET_ID = '1g6ibbyoc8NmK788oqyxg2EJJGRfRrnPIuULvCRNaEjU';

// Enerji verileri için ayrı spreadsheet (eski sistem)
const ENERGY_SPREADSHEET_ID = '1ncEnE6vG76HIXFzJRRWz2yehiN2oolg4t2pzfbKt2vI';
...
function getLatestEnergyMotorHours(ss, motor) {
  // Önce enerji spreadsheet'inden oku
  try {
    const energySS = SpreadsheetApp.openById(ENERGY_SPREADSHEET_ID);
    const energyHours = getLatestEnergyMotorHoursFromSheets(energySS, motor);
    if (energyHours > 0) return energyHours;
  } catch (error) {
    Logger.log('Enerji spreadsheet okunamadi: ' + error.toString());
  }
  
  // Enerji spreadsheet okunamazsa API'yi dene
  const apiHours = getLatestEnergyMotorHoursFromApi(motor);
  if (apiHours > 0) return apiHours;
  
  // Son çare olarak kendi spreadsheet'imize bak
  ss = ss || getSpreadsheet();
  return getLatestEnergyMotorHoursFromSheets(ss, motor);
}
...
function getMotorHours(ss) {
  ...
  const motors = MOTORS.map(function(motor) {
    ...
    const periodicPlan = calculatePeriodicPlan(currentHours);
  ...
  writeSettingsPeriodicMaintenanceTable(ss, motors);
...
function writeSettingsPeriodicMaintenanceTable(ss, motors) {
  ss = ss || getSpreadsheet();
  const sheet = ensureSettingsSheet(ss);
  const startRow = 6;
  const startColumn = 4;
  ...
  const rows = (motors || []).map(function(item) {
    const plan = item.periodicPlan || {};
    let status = 'Normal';
    
    if (plan.needsMaintenance) {
      status = 'Bakim zamani geldi!';
    } else if (plan.remainingHours <= PERIODIC_URGENT_WARNING_HOURS) {
      status = PERIODIC_URGENT_WARNING_HOURS + ' saat kala ACIL UYARI';
    } else if (plan.warnsMaintenance) {
      status = PERIODIC_WARNING_HOURS + ' saat kala uyar';
    }
    ...
    return [
      ...
      item.currentHours || 0,
      plan.lastThreshold || '',
      plan.lastMaintenanceType || '',
      plan.nextThreshold || '',
      plan.nextMaintenanceType || '',
      plan.remainingHours !== undefined ? plan.remainingHours : '',
      status
    ];
  });
...
function getLatestEnergyMotorHoursInfo(ss, motor) {
  ...
  try {
    ...
    const energyHours = getLatestEnergyMotorHoursFromSheets(energySS, motor);
    if (energyHours > 0) {
      return { hours: energyHours, source: 'Enerji Spreadsheet', note: 'ID: ' + ENERGY_SPREADSHEET_ID };
    }
  } catch (error) {
    Logger.log('Enerji spreadsheet info okunamadi: ' + error.toString());
  }
  ...
  // Kendi spreadsheet'imize bak
  ss = ss || getSpreadsheet();
  const local = getLatestEnergyMotorHoursFromSheets(ss, motor);
  if (local > 0) {
    return { hours: local, source: 'Bakim Takip Enerji sayfasi', note: '' };
  }

  return { hours: 0, source: 'Bulunamadi', note: 'Enerji verisi yok' };
}
